...
class QIMF_2D:
    ...
    def __init__(self, hx=0, hy=0.9045, J=0.4, N=N):
        self.hx = hx
        self.hy = hy
        self.J = J
        self.N = N
        self.H = self.get_hamiltonian(hx, hy, J)
        self.U0 = self.get_evolution_segment(time_step)

    def get_hamiltonian(self,hx,hy,J):
        self.X_tuples = [("X", [i], hx) for i in range(0, N)]
        self.Y_tuples = [("Y", [i], hy) for i in range(0, N)]
        connectors=[[i, i + 1] for i in range(3)] 
        connectors.extend([[4+i,5+i] for i in range(3)])
        connectors.extend([[8+i,9+i] for i in range(3)])
        connectors.extend([[i,i+4] for i in range(4)])
        connectors.extend([[4+i,8+i] for i in range(4)])
        self.XX_tuples = [("XX", pair_index, J) for pair_index in connectors]
...
class analog_QITF(QIMF_2D):
    def __init__(self, exact_model, delta=0.001,eta=0.001):
...
if __name__ == "__main__":
    exact_model=QIMF_2D(hx=0.809,hy=0.9045,J=1,N=N)
    ...
    plt.legend(loc=1)
    plt.title('2D-QIMF model, long-time evolution')
    plt.grid()
    plt.show()